		...
		if trimmed == "" then
			...
			for i = 1, #trimmed do
				...
				elseif ch == "}" then
					...
					if not found_non_brace then
						leading_closes = leading_closes + 1
					end
...
local function scope_icon(key)
	if SCOPE_ICONS[key] then
		return SCOPE_ICONS[key]
	end
...
function M.get_scope_parts()
	...
	if vim.bo[bufnr].filetype ~= "pdxscript" then
		return nil
	end
	...
	for i = #lines, 1, -1 do
		...
		for _ = 1, opens do
			depth = depth - 1
			if depth < 0 then
				local key = line:match("^%s*([%w_.:]+)%s*[<>!?]*=%s*{") or line:match("^%s*([%w_.:]+)%s*{")
				if key then
					table.insert(parts, 1, { icon = scope_icon(key), key = key, hl = M.icon_hl(key) })
				end
				depth = 0
			end
		end
...
function M.icon_hl(key)
	if ICON_HL_MAP[key] then
		return ICON_HL_MAP[key]
	end
	if key:find("%.") then
		return "PdxIconEvent"
	end
...
function M.ensure_modeline(bufnr)
	...
	local lines = vim.api.nvim_buf_get_lines(bufnr, 0, -1, false)
	for _, line in ipairs(lines) do
		if line:find("vim: set filetype=pdxscript", 1, true) then
			return
		end
	end
