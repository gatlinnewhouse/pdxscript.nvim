...
function M.maybe_start(bufnr)
	...
	if type(auto) == "function" and not auto(bufnr) then
		return
	end
	...
	if bin_dir:sub(-1) ~= "/" then
		bin_dir = bin_dir .. "/"
	end
	local cmd = bin_dir .. bin_name

	if vim.fn.executable(cmd) == 0 then
		vim.notify("pdxscript-lsp: binary not found: " .. cmd, vim.log.levels.WARN, { title = "pdxscript.nvim" })
		return
	end
	...
	vim.lsp.start({
		...
		reuse_client = function(client, conf)
			return client.name == "pdxscript-lsp" and client.config.root_dir == conf.root_dir
		end,
	})
end
